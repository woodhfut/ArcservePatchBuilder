...
class UDPStatusConsumer(WebsocketConsumer):
    ...
    def _create_patch(self, name):
        ...
        if len(results)==0 or all([x.get()[1] for x in results]):
            ...
            print('copy {} to apm folder {}'.format(fixpath, apm))
            ...
            if os.path.exists(os.path.join(apm,fixname+'_1')):    
                #shutil.rmtree(os.path.join(apm, fixname), onerror=remove_readonly)
                subprocess.run('cmd /c rd /S /Q ' + os.path.join(apm,fixname+'_1'))
                
            shutil.copytree(fixpath, os.path.join(apm, fixname))
            ...
            with utils.Lock_obj:
                ...
                cmd = '{} -t {}'.format(createpatch, os.path.join(apm,fixname))
                print(cmd)
                self.send(json.dumps({
                    'msgType': 'PatchStatus',
            ...
            exepath = os.path.join(apm,fixname+'_1\\MQA\\Build.000\\'+fixname+'.exe')
    ...
    def _clean_up(self):
        ...
        try:
            ...
            if os.path.exists(fixfolder+'_1'):
                shutil.rmtree(fixfolder+'_1', ignore_errors=True)    
